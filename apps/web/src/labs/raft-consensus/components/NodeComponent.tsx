...
export function NodeComponent({ id, state, term, timer, x, y, onKill, onRevive }: NodeProps) {
  ...
  return (
    <motion.div
      className={`absolute w-28 h-28 sm:w-32 sm:h-32 rounded-none flex flex-col items-center justify-center font-medium border-4 shadow-[4px_4px_0px_0px_rgba(28,25,23,1)] cursor-pointer transition-colors duration-500 ${theme.bg} ${theme.border} ${theme.text}`}
      ...
      layout
    >
      <span className="text-lg sm:text-xl font-bold tracking-tight">{id}</span>
